    ...
    >
      <div className="bg-white rounded-[18px] max-w-[680px] w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="p-8">
          ...
          {event.fullDesc && (
            <p className="text-[15px] text-[#6b6b66] leading-[1.7] mb-6">{event.fullDesc}</p>
          )}

          {event.tags && event.tags.length > 0 && (
            <div className="flex gap-2 flex-wrap mb-6">
              {event.tags.map(tag => (
                <span key={tag} className="text-[11px] font-medium text-[#6b6b66] bg-[#f0f0ec] px-2.5 py-1 rounded-md">
                  {tag}
                </span>
              ))}
            </div>
