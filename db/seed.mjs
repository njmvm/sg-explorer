/**
 * Seed script: reads data/content.js + hardcoded trips and inserts into Neon DB.
 * Run with: node db/seed.js
 * Requires DATABASE_URL in .env.local (run `vercel env pull .env.local` first).
 */
...
// Dynamic import for content.js (ESM)
const { activities, events, trips } = await import('../data/content.js')

const sql = neon(process.env.DATABASE_URL)

// Extra trips hardcoded in app/trips/page.js — include them in the seed
const extraTrips = [
  {
...
async function seed() {
  ...
  for (const e of events) {
    await sql`
      ...
      VALUES (${e.id}, ${e.tab}, ${e.category}, ${e.title}, ${e.location}, ${e.time}, ${e.price}, ${e.date.month}, ${e.date.day}, ${e.date.label}, ${e.image}, ${e.fullDesc}, ${e.tags}, ${e.lat}, ${e.lng}, ${e.website})
      ON CONFLICT (id) DO UPDATE SET
        tab = EXCLUDED.tab,
        category = EXCLUDED.category,
        title = EXCLUDED.title,
  ...
  console.log(`  ${allTrips.length} trips seeded.`)
